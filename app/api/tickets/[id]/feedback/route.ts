...
export const POST = withAuth(
  { role: "viewer" },
  async (request, ctx: RouteContext<"/api/tickets/[id]/feedback">, viewer) => {
    // The inverse of `PATCH /api/tickets/[id]`: staff are refused here. An
    // `orphan` — a CUSTOMER login with no linked `Customer` row
    // (`lib/ticket-access.ts`) — is refused too, and is never resolved by
    ...
    // `app/api/tickets/[id]/comments/route.ts`. A 403 confirms the id exists.
    const ticket = await prisma.ticket.findFirst({
      where: { id, customerId: viewer.customerId, ...NOT_DELETED },
